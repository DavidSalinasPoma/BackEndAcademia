...
class ReflexionController extends Controller
{
    ...
    {
        $carrera = Reflexion::all()->load('usuarios');
        $data = array(
            'code' => 200,
            'status' => 'success',
            'carrera' => $carrera
    ...
    {
        $carrera = Reflexion::find($idCarrera);
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($params) && !empty($paramsArray)) {
            ...
            if ($validate->fails()) { // en caso si los datos fallan la validacion
                ...
                $reflexion->titulo = $paramsArray['titulo'];
                $reflexion->descripcion = $paramsArray['descripcion'];
                // htmlentities($paramsArray['descripcion']);
                $reflexion->imagen = $paramsArray['imagen'];
                $reflexion->tipo = $paramsArray['tipo'];

                $reflexion->usuarios_id = $user->sub;
                ...
                $data = array(
                    'status' => 'success',
                    'code' => 200,
                    'message' => 'La lectura se  creado correctamente.',
                    'idUltimo' => $reflexion->id
                );
            }
        } else {
            $data = array(
    ...
    public function update(Request $request)
    {
        ...
        if (!empty($paramsArray)) {
            ...
            if ($validate->fails()) { // en caso si los datos fallan la validacion
                ...
                try {
                    $registrados = Reflexion::where('id', $idCarrera)->update($paramsArray);
                    $data = array(
                        'status' => 'success',
                        'code' => 200,
        ...
        if (!empty($params) && !empty($paramsArray)) {
            ...
            if ($validate->fails()) { // en caso si los datos fallan la validacion
                ...
                try {
                    ...
                    // 5.-Crear el usuario
                    $carrera_update = Reflexion::where('id', $params->id)->update(['estado' => $params->estado]);
                    if ($params->estado === 1) {
                        $data = array(
                            'status' => 'success',
                            'code' => 200,
                            'message' => 'La Lectura se habilito correctamente.',
                        ...
                        $data = array(
                            'status' => 'success',
                            'code' => 200,
                            'message' => 'La Lectura se deshabilito correctamente.',
                            'estado' => $params->estado,
                            'carrera' => $carrera_update
                        );
                    }
