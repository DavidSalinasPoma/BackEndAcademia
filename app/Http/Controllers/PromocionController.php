...
class PromocionController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('api.auth', ['except' => ['index', 'show', 'getImagen', 'bajaPromo', 'destroyImagen']]);
    }

    // INDEX sirve para sacar todos los registrol de promociones  de la base de datos
    public function index()
    {
        $promocion = Promocion::all()->load('usuarios');
        $data = array(
            'code' => 200,
            'status' => 'success',
            'oferta' => $promocion
    ...
    public function update(Request $request)
    {
        ...
        if (!empty($paramsArray)) {
            ...
            if ($validate->fails()) { // en caso si los datos fallan la validacion
                ...
                try {
                    $registrados = Promocion::where('id', $idPromocion)->update($paramsArray);
                    $data = array(
                        'status' => 'success',
                        'code' => 200,
    ...
    public function bajaPromo(Request $request)
    {
        ...
        if (!empty($params) && !empty($paramsArray)) {
            ...
            if ($validate->fails()) { // en caso si los datos fallan la validacion
                ...
                try {
                    ...
                    // 5.-Crear el usuario
                    $oferta_update = Promocion::where('id', $params->id)->update(['estado' => $params->estado]);
                    if ($params->estado === 1) {
                        $data = array(
                            'status' => 'success',
                            'code' => 200,
                            'message' => 'La oferta se habilito correctamente.',
                            'estado' => $params->estado,
                            'oferta' => $oferta_update
                        );
                    } else {
                        $data = array(
                            'status' => 'success',
                            'code' => 200,
                            'message' => 'La oferta se deshabilito correctamente.',
                            'estado' => $params->estado,
                            'oferta' => $oferta_update
                        );
                    }
